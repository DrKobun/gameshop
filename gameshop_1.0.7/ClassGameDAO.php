...
?>
    ...
    public function mostrarJogoEspecifico($id)
    {

        try 
        {
            $pdo = Conexao::getInstance();
            $sql = "SELECT nome, preco, descricao, path FROM game WHERE id=:id";
            $stmt = $pdo->prepare($sql);
    ...
    public function tipoContaUsuario($id) 
    {
        try 
        {
            $pdo = Conexao::getInstance();
            $sql = "SELECT tipoConta FROM usuarios WHERE id = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->bindValue(':id', $id);
            $stmt->execute();
            ...
            // Retorna o tipo de conta se houver resultado
            if ($resultado !== false) {
                return $resultado['tipoConta'];
            } else {
                // Se não houver resultado, você pode retornar null ou lançar uma exceção, dependendo do caso
                return null;
            }
        } 
        catch (PDOException $erro) 
        {
            // Em caso de erro, você pode retornar uma mensagem de erro ou lançar uma exceção
            return $erro->getMessage();
        }          
	}
    
    function adicionarCarrinho($idProduto, $idUsuario)
    {
        try 
        {
            $pdo = Conexao::getInstance();
            $sql = "INSERT INTO carrinho (IdJogo, IdUsuario_FK) VALUES (?, ?)";
            $stmt = $pdo->prepare($sql);
        ...
        {
            echo "Erro ao adicionar jogo ao carrinho: " . $erro->getMessage();
        }
    }
